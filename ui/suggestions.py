    ...
    try:
        ...
        if status!="all":
            c.execute("""SELECT id,file_path,action,reason,confidence,risk,status
                        ...
                        ORDER BY confidence DESC LIMIT 20""",(status,))
        else:
            c.execute("""SELECT id,file_path,action,reason,confidence,risk,status
                        FROM suggestions ORDER BY confidence DESC LIMIT 20""")
        return c.fetchall()
    except: return []
    ...
    def __init__(self, parent, data, reload_cb, **kw):
        ...
        if status=="pending":
            ctk.CTkButton(bot, text="Approve", width=84, height=28,
                ...
                command=lambda:(update(sid,"approved"), self._bust_cache(), reload_cb())).pack(side="right", padx=(6,0))
            ctk.CTkButton(bot, text="Reject", width=76, height=28,
                ...
                command=lambda:(update(sid,"rejected"), self._bust_cache(), reload_cb())).pack(side="right")
...
class SuggestionsPanel(ctk.CTkFrame):
    ...
    def _load(self):
        ...
        if _cache[f] is not None:
            self._render(_cache[f], get_total(f))
            return
    ...
    def _render(self, rows, total):
        ...
        if not rows:
            ...
            return
        for r in rows:
            SRow(self._scroll, r, self._load).pack(fill="x", pady=6)